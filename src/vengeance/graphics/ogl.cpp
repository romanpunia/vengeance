...
namespace
{
	...
	static void OGL_CopyTextureFace2D_3_0(Vitex::Trigonometry::CubeFace Face, GLuint SrcName, GLuint DestName, GLint Width, GLint Height)
	{
		GLint LastFrameBuffer = 0;
		glGetIntegerv(GL_FRAMEBUFFER_BINDING, &LastFrameBuffer);
...
namespace Vitex
{
	namespace Graphics
	{
		namespace OGL
		{
			...
			Trigonometry::Vertex* OGLMeshBuffer::GetElements(GraphicsDevice* Device) const
			{
			...
			Trigonometry::SkinVertex* OGLSkinMeshBuffer::GetElements(GraphicsDevice* Device) const
			{
				VI_ASSERT(Device != nullptr, "graphics device should be set");
			...
			void OGLDevice::SetScissorRects(uint32_t Count, Trigonometry::Rectangle* Value)
			{
			...
			void OGLDevice::GetScissorRects(uint32_t* Count, Trigonometry::Rectangle* Out)
			{
				GLint Rect[4];
			...
			void OGLDevice::ImTransform(const Trigonometry::Matrix4x4& Transform)
			{
			...
			ExpectsGraphics<void> OGLDevice::UpdateBuffer(MeshBuffer* Resource, Trigonometry::Vertex* Data)
			{
				VI_ASSERT(Resource != nullptr, "resource should be set");
			...
			ExpectsGraphics<void> OGLDevice::UpdateBuffer(SkinMeshBuffer* Resource, Trigonometry::SkinVertex* Data)
			{
				VI_ASSERT(Resource != nullptr, "resource should be set");
				VI_ASSERT(Data != nullptr, "data should be set");
			...
			ExpectsGraphics<void> OGLDevice::CopyTexture2D(RenderTargetCube* Resource, Trigonometry::CubeFace Face, Texture2D** Result)
			{
				VI_ASSERT(Resource != nullptr, "resource should be set");
				VI_ASSERT(Result != nullptr, "result should be set");
			...
			ExpectsGraphics<void> OGLDevice::CopyTexture2D(MultiRenderTargetCube* Resource, uint32_t Cube, Trigonometry::CubeFace Face, Texture2D** Result)
			{
				VI_ASSERT(Resource != nullptr, "resource should be set");
				VI_ASSERT(Result != nullptr, "result should be set");
			...
			ExpectsGraphics<void> OGLDevice::CopyTextureCube(MultiRenderTargetCube* Resource, uint32_t Cube, TextureCube** Result)
			{
				...
				if (!*Result)
				{
					Texture2D* Faces[6] = { nullptr };
					for (uint32_t i = 0; i < 6; i++)
						CopyTexture2D(Resource, Cube, (Trigonometry::CubeFace)i, &Faces[i]);
			...
			ExpectsGraphics<void> OGLDevice::CubemapFace(Cubemap* Resource, Trigonometry::CubeFace Face)
			{
				VI_ASSERT(Resource != nullptr, "resource should be set");
				VI_ASSERT(Resource->IsValid(), "resource should be valid");
